...
class NadoClient(BaseExchangeClient):
    ...
    async def fetch_bbo_prices(self, contract_id: str) -> Tuple[Decimal, Decimal]:
        ...
        try:
            # Convert contract_id to integer (product_id)
            product_id = int(contract_id)

            self.logger.log(f"Attempting to fetch orderbook for product_id={product_id}", "DEBUG")

            # Try to get order book - try multiple methods
            order_book = None

            # Method 1: Try get_orderbook with product_id as int
            try:
                self.logger.log(f"Method 1: engine_client.get_orderbook(ticker_id={product_id}, depth=1)", "DEBUG")
                order_book = self.client.context.engine_client.get_orderbook(ticker_id=product_id, depth=1)
                self.logger.log(f"Method 1 succeeded!", "DEBUG")
            except Exception as e1:
                self.logger.log(f"Method 1 failed: {e1}", "DEBUG")

            # Method 2: Try using ticker symbol directly (ETH-PERP)
            if order_book is None:
                try:
                    ticker_symbol = f"{self.config.ticker}-PERP"
                    self.logger.log(f"Method 2: engine_client.get_orderbook(ticker_id='{ticker_symbol}', depth=1)", "DEBUG")
                    order_book = self.client.context.engine_client.get_orderbook(ticker_id=ticker_symbol, depth=1)
                    self.logger.log(f"Method 2 succeeded!", "DEBUG")
                except Exception as e2:
                    self.logger.log(f"Method 2 failed: {e2}", "DEBUG")

            # Method 3: Try using ticker symbol with USDT0 suffix
            if order_book is None:
                try:
                    ticker_symbol_usdt = f"{self.config.ticker}-PERP_USDT0"
                    self.logger.log(f"Method 3: engine_client.get_orderbook(ticker_id='{ticker_symbol_usdt}', depth=1)", "DEBUG")
                    order_book = self.client.context.engine_client.get_orderbook(ticker_id=ticker_symbol_usdt, depth=1)
                    self.logger.log(f"Method 3 succeeded!", "DEBUG")
                except Exception as e3:
                    self.logger.log(f"Method 3 failed: {e3}", "DEBUG")

            # Method 4: Try using market API
            if order_book is None:
                try:
                    self.logger.log(f"Method 4: client.market.get_orderbook(product_id={product_id})", "DEBUG")
                    order_book = self.client.market.get_orderbook(product_id=product_id)
                    self.logger.log(f"Method 4 succeeded!", "DEBUG")
                except Exception as e4:
                    self.logger.log(f"Method 4 failed: {e4}", "DEBUG")

            if not order_book:
                self.logger.log(f"Failed to get orderbook for product_id={product_id}", "ERROR")
                return Decimal(0), Decimal(0)

            self.logger.log(f"Orderbook fetched successfully, extracting bids/asks", "DEBUG")
            ...
            if not bids or not asks:
                self.logger.log("No bids or asks in orderbook", "ERROR")
                return Decimal(0), Decimal(0)
            ...
            self.logger.log(f"Best bid: {best_bid}, Best ask: {best_ask}", "DEBUG")
    ...
    async def get_order_price(self, direction: str) -> Decimal:
        ...
        if direction == 'buy':
            # For buy POST_ONLY orders, price must be below best bid
            # Use a larger spread to ensure we don't cross the book
            order_price = best_bid - (self.config.tick_size * Decimal('2'))
        else:
            # For sell POST_ONLY orders, price must be above best ask
            # Use a larger spread to ensure we don't cross the book
    ...
    async def place_open_order(self, contract_id: str, quantity: Decimal, direction: str) -> OrderResult:
        ...
        while retry_count < max_retries:
            try:
                ...
                # Determine order price - POST_ONLY requires price to NOT cross the spread
                # For buy orders: price must be < best_bid (on the bid side)
                # For sell orders: price must be > best_ask (on the ask side)
                if direction == 'buy':
                    # Buy order price must be below best bid to be POST_ONLY
                    # Use a larger spread to ensure we don't cross the book
                    order_price = best_bid - (self.config.tick_size * Decimal('2'))
                else:
                    # Sell order price must be above best ask to be POST_ONLY
                    # Use a larger spread to ensure we don't cross the book
                    order_price = best_ask + (self.config.tick_size * Decimal('2'))

                # Round to tick size to ensure price is valid
    ...
    async def place_close_order(self, contract_id: str, quantity: Decimal, price: Decimal, side: str) -> OrderResult:
        ...
        while retry_count < max_retries:
            try:
                ...
                # Adjust order price based on market conditions - POST_ONLY requires
                # sell orders: price > best_ask
                # buy orders: price < best_bid
                adjusted_price = price
                if side.lower() == 'sell':
                    # For sell POST_ONLY orders, price must be above best ask
                    # Use a much larger spread to ensure we don't cross the book
                    if price <= best_ask:
                        adjusted_price = best_ask + (self.config.tick_size * Decimal('10'))
                    else:
                        # Even if price > best_ask, add more buffer to be safe
                        adjusted_price = price + (self.config.tick_size * Decimal('2'))
                elif side.lower() == 'buy':
                    # For buy POST_ONLY orders, price must be below best bid
                    # Use a much larger spread to ensure we don't cross the book
                    if price >= best_bid:
                        adjusted_price = best_bid - (self.config.tick_size * Decimal('10'))
                    else:
                        # Even if price < best_bid, add more buffer to be safe
                        adjusted_price = price - (self.config.tick_size * Decimal('2'))
